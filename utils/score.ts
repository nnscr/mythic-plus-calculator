 ...
 */
export function calculateBaseScore(dungeon: DungeonDefinition, level: number, duration: number): Decimal {
    if (level <= 0) {
    ...
    if (level > 20 && duration >= dungeon.plus1) {
        // apparently overtimed keys above level 20 are treated as if they were level 20
        level = 20;
    }
 ...
 */
export function calculateScores(dungeon: DungeonDefinition, dungeonTime: OverallDungeonTime): CalculatedScores {
    ...
    const fortifiedRawScore = calculateBaseScore(dungeon, dungeonTime.Fortified.level, dungeonTime.Fortified.duration);
    const fortifiedScore = fortifiedRawScore.times(10).round().dividedBy(10);

    const higherScore = tyrannicalScore.greaterThan(fortifiedScore) ? "Tyrannical" : "Fortified";

    const weightHeigher = 1.5;
    const weightLower = 0.5;

    const weightTyrannical = higherScore === "Tyrannical" ? weightHeigher : weightLower;
    const weightForitifed = higherScore === "Fortified" ? weightHeigher : weightLower;

    // weighted scores are calculated as actual floats, because why not
    const weightedRawScoreTyrannical = new Decimal(tyrannicalScore.toNumber() * weightTyrannical);

    const weightedRawScoreFortified = new Decimal(fortifiedScore.toNumber() * weightForitifed);
    ...
    const totalRawScore = new Decimal(weightedRawScoreFortified.toNumber() + weightedRawScoreTyrannical.toNumber());
    ...
    return {
        ...
        totalRawScore: new Decimal(tyrannicalRawScore.toNumber() * weightTyrannical).plus(
            new Decimal(fortifiedRawScore.toNumber() * weightForitifed)
        ),
    };
}

export function calculatePlayerScore(scores: Record<DUNGEON_SHORTS, CalculatedScores>): Decimal {
...
export function getTimerBonus(dungeon: DungeonDefinition, duration: number): Decimal | null {
